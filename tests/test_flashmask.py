...
import flash_maskattn_2_cuda as flash_attn_cuda
...
#                 #     startend_row_indices[b, k, i, 1] = seq_len
#                 # else:
#                     startend_row_indices[b, k, i, 0] = seq_len - 2
#                     startend_row_indices[b, k, i, 1] = seq_len
#                 # startend_row_indices[b, k, i, 0] = seq_len
...
def flashmask_to_densemask(startend_row_indices, dtype, causal=True):
    ...
    m = torch.ones((bz, num_head, seq_len, seq_len), dtype=dtype, device="cuda")
...
def generate_inputs(batch_size, seq_len, num_heads, head_dim, dtype=torch.bfloat16):
    """生成测试输入数据"""
    torch.random.manual_seed(42)
    ...
    # out = torch.zeros(batch_size, seq_len, num_heads, head_dim, device=device, dtype=dtype)
    return q, k, v
    ...
    for b in range(batch_size):
        for k in range(num_heads):
            for i in range(seq_len):
                ...
                #     startend_row_indices[b, k, i, 0] = seq_len
                #     startend_row_indices[b, k, i, 1] = seq_len
                #     startend_row_indices[b, k, i, 2] = 0
                #     startend_row_indices[b, k, i, 3] = i
                # elif i >= (seq_len - 32 - mask_len) and i < (seq_len - 32):
                    startend_row_indices[b, k, i, 0] = seq_len - 32
                    startend_row_indices[b, k, i, 1] = seq_len
                #     startend_row_indices[b, k, i, 2] = 0
                ...
                #     startend_row_indices[b, k, i, 1] = seq_len
                #     startend_row_indices[b, k, i, 2] = 0
                #     startend_row_indices[b, k, i, 3] = seq_len - 32
                # startend_row_indices[b, k, i, 0] = seq_len - 2
    return startend_row_indices
...
def test_flashmask():
    ...
    q, k, v = generate_inputs(batch_size, seq_len, num_heads, head_dim)
    q1, k1, v1 = q.detach().clone().transpose(1,2), k.detach().clone().transpose(1,2), v.detach().clone().transpose(1,2)
    ...
    startend_row_indices = generate_causal_mask(batch_size, seq_len, num_heads)
    output = flash_attn_cuda.flashmask_fwd(
            q, k, v,
            startend_row_indices, None, None,
            0.0, True, True, True, 
    )
    torch.cuda.synchronize()
    softmax_lse = output[1]
    print(softmax_lse.size())

    m_bool = flashmask_to_densemask(startend_row_indices, torch.bool)
    # print(m_bool)
    out_flash_attn = torch.nn.functional.scaled_dot_product_attention(q1, k1, v1, m_bool)
    torch.cuda.synchronize()

    assert out_flash_attn.shape == output[0].transpose(1,2).shape, "Output shapes do not match"
    abs_max_error = torch.abs(out_flash_attn - output[0].transpose(1,2)).max().item()
    print(f"Absolute Max error: {abs_max_error:.6f}")
    grad_out = torch.randn_like(out_flash_attn)
    
    # 反向传播
    out_flash_attn.backward(grad_out)
    torch.cuda.synchronize()
    grad_q_pytorch = q1.grad.clone()
    grad_k_pytorch = k1.grad.clone()
    grad_v_pytorch = v1.grad.clone()
    # print(grad_q_pytorch.size())
    print(out_flash_attn.size())
    output_bwd = flash_attn_cuda.flashmask_bwd(
            q, k, v, out_flash_attn.transpose(1,2), softmax_lse,
            startend_row_indices, grad_out.transpose(1,2), None, None, None,
            0.0, True
    )
    torch.cuda.synchronize()
    print(output_bwd[0].size())
    dq_abs_max_error = torch.abs(grad_q_pytorch - output_bwd[0].transpose(1, 2)).max().item()
    dk_abs_max_error = torch.abs(grad_k_pytorch - output_bwd[1].transpose(1, 2)).max().item()
    dv_abs_max_error = torch.abs(grad_v_pytorch - output_bwd[2].transpose(1, 2)).max().item()
    print(f"dq Absolute Max error: {dq_abs_max_error:.6f}")
    print(f"dk Absolute Max error: {dk_abs_max_error:.6f}")
    print(f"dv Absolute Max error: {dv_abs_max_error:.6f}")
    ...
    # print(output[0].transpose(1,2))
    # print(out_flash_attn)
